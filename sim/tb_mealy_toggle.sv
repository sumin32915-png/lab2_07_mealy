...
module tb_mealy_toggle;
    ...
    task finish;
    begin
        $display("LAB2_PASS mealy_toggle checks=%d", checks);
        $finish;
    end
    endtask
    ...
    initial begin
        ...
        enable = 1; step; check({state, value} === 3'b101, "transition to 1");
        bit_in = 0; #1; check({state, value} === 3'b100, "S1 input zero immediately");
        step; check(state === 1'b1, "input zero holds S1");
        bit_in = 1; #1; check({state, value} === 3'b101, "S1 pre-edge value");
        step; check({state, value} === 3'b010, "transition to 0");
        step; check(state === 1'b1, "toggle again");
        rst = 1; step; check({state, value} === 3'b010, "reset state with input one");
        bit_in = 0; #1; check({state, value} === 3'b000, "clear combinational output"); finish;
    end
endmodule